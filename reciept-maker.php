...
// require_once WVR_PLUGIN_PATH . '/vendor/autoload.php'; // Adjust the path to Composer autoload

use Dompdf\Dompdf;
use Dompdf\Options;

function generate_receipt_pdf_dom($booking_id, $payment_id, $client_id)
{
    global $wpdb;
    ...
    $dompdf->set_option('defaultFont', 'Open Sans'); // Set default font
    ...
    // Add content
    $html = '
    <link href="https://fonts.googleapis.com/css2?family=Prata&family=Open+Sans&display=swap" rel="stylesheet">
    <style>
        h1 { font-family: Prata, serif; color: #ffffff; font-weight: bold; }
        h2 { font-family: Prata, serif; color: #242424; font-weight: bold; }
        p { font-family: "Open Sans", sans-serif; color: #7a7a7a; }
        .header { background-color: #001540; color: #ffffff; padding: 10px; text-align: center; }
        .section-title { font-family: Prata, serif; color: #003399; font-size: 18px; font-weight: bold; margin-bottom: 0; }
        .details { font-family: "Open Sans", sans-serif; color: #7a7a7a; }
    </style>
    ...
    // Output the PDF
    $dompdf->stream('dsmart_booking_receipt_' . $booking_id . '_' . time() . '.pdf', ['Attachment' => 1]); // 'Attachment' => 1 forces download
}
